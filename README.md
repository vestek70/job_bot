...
1. `search_jobs.py` — ищет вакансии через открытый API Adzuna (покрывает Бразилию)
   и дополнительно через Remotive и Arbeitnow (`extra_sources.py`) — публичные API
...
|---|---|---|---|
| Adzuna | Бразилия, агрегатор, офисные + удалённые | Да (`ADZUNA_APP_ID`/`ADZUNA_APP_KEY`) | всегда включён (обязательный) |
...
У Remotive и Arbeitnow нет надёжного поиска по ключевым словам на португальском
(контент в основном на английском) — вместо перевода запроса эти два источника
фильтруются по релевантности "fullstack" в названии/тегах, поэтому всегда
приносят fullstack-вакансии независимо от ключевого слова, переданного в
`python main.py "..."`. Так как обе площадки — по сути только удалённые
вакансии (или мы оставляем только `remote=true`), они естественно сочетаются
с фильтром локации проекта.
...
Даже с 3 источниками по умолчанию включены два фильтра: уровень (junior/pleno)
и локация (Florianópolis или удалённо) — оба сокращают итоговый список. На узкий
запрос вроде "desenvolvedor fullstack junior" всё ещё возможно получить мало
вакансий или ноль в конкретный день — это нормально, площадки обновляются со
временем.
