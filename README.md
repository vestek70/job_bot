...
Удалённые источники (Remotive/Arbeitnow/RemoteOK/Jobicy) не имеют надёжного
поиска по ключевым словам на португальском (контент в основном на английском) —
вместо перевода запроса они фильтруются по релевантности к разработке
(`RELEVANCE_KEYWORDS` в `config.py` — fullstack, backend, frontend, react, node,
python, php, laravel, vue, angular, typescript, javascript, "desenvolvedor",
"programador", "software engineer/developer" и т.д.). Раньше фильтр был только
по слову "fullstack" — это отсекало backend/frontend/react/python-вакансии, на
которые ты тоже можешь откликаться. Теперь список широкий. Список можно
переопределить через `RELEVANCE_KEYWORDS` в `.env` (термины через запятую).
